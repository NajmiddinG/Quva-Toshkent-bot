...
async def handle_all_messages(message: types.Message, state: FSMContext):
    ...
    if (sender_id not in get_users_id('Admin') and sender_id not in get_users_id('Haydovchi')) and str(message.chat.id) == str(main_group):
        await forward_message_to_bot(message)
    if (sender_id not in get_users_id('Admin') and sender_id not in get_users_id('Haydovchi')) and str(message.from_user.id) == str(message.chat.id):
